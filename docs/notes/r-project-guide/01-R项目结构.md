...
**【已弃用】**：在renv配置环境中.Rprofile会在project打开时运行source("renv/activate.R")，在服务器上会导致较长加载打开时间（10+分钟）。且服务器上project打开后运行速度很慢。故本项目不使用该renv环境和项目project设置。

<hr>

## 替换流程：

在本地R中打开并运行Z:\projects\onc-prj-shr-a1811\sub-csr\shr-a1811-ii-206\01_setup\00-setup.R 文件（注意在运行时需保持在该文件页面，避免抓取项目路径失败），需确保基础package都已安装至本地环境中。

```R

# 加载必要的包 - 检查安装后加载 （已包含至setup.R中）
if (!require("metacore")) install.packages("metacore")
if (!require("metatools")) install.packages("metatools")
if (!require("dplyr")) install.packages("dplyr")
if (!require("haven")) install.packages("haven")
if (!require("purrr")) install.packages("purrr")
if (!require("admiral")) install.packages("admiral")
if (!require("xportr")) install.packages("xportr")
...
储存在Environment的spec_metacore变量中
...
**设置project打开环境**：打开project会自动restore setup信息
【由lead programmer创建和控制，其他人不建议修改】
...
5. 【仅Project适用】规则设置：Tools-> Project Options...-> General <br>
   ![alt text](image-25.png) <br>
6. 建议设置：Tools-> Global Options...-> General -> Basic<br>
